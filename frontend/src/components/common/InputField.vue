<template>
  <div :class="['', {'input-error': !isValidate}, {success: successful}]" :style="{width: styleWidth}" @focus="focus">
    <textarea
        ref="textarea"
        :class="['', {block: block}]"
    ...
    <input
        ref="input"
        :class="['', {block: block}, {'text-right': textRight}, {'text-right': textRight}, { 'lengthCount' : lengthCount }]"
    ...
    <span v-if="lengthCount" class="input-maxlength"><em>{{ value ? value.length : 0 }}</em> / 30</span>
